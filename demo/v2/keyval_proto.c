...
/* public domain crc32 code from: http://csbruce.com/software/crc32.c */
static uint32_t crc32(unsigned long inCrc32, const void* buf, size_t bufLen)
{
...
size_t prep_req(keyval_proto_req_t* req, const char* key)
{
    ...
    req->hdr.crc = 0;
    req->hdr.crc = htonl(crc32(0, req, len));
...
size_t prep_resp(keyval_proto_resp_t* resp, const char* val)
{
    ...
    resp->hdr.crc = 0;
    resp->hdr.crc = htonl(crc32(0, resp, len));
...
int handle_msg(int sock, uint8_t* in_buf, ssize_t len, struct sockaddr_in* other_addr, socklen_t sock_len)
{
    ...
    keyval_proto_hdr_t* header = (keyval_proto_hdr_t*)in_buf;
    uint32_t crc_in = ntohl(header->crc);
    header->crc = 0;
    uint32_t crc_calc = crc32(0, header, len);
